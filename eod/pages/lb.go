...
func (p *Pages) LbHandler(c sevcord.Ctx, params string) {
	...
	if qu == nil {
		err = p.db.Select(&vals, `SELECT "user", `+lbSortCode[parts[2]]+` cnt FROM inventories WHERE guild=$1 AND lbbanned=0 ORDER BY cnt DESC`, c.Guild())
	} else {
		...
		err = p.db.Select(&vals, `SELECT "user", `+sort+` cnt FROM inventories WHERE guild=$1 AND lbbanned=0 ORDER BY cnt DESC`, c.Guild(), pq.Array(qu.Elements))
	}
	if err != nil {
		p.base.Error(c, err)
		return
