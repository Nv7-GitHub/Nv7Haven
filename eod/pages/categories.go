...
var catListSortSql = map[string]string{
	"name":  "name",
	"count": "cardinality(elements) DESC",
	"found": "common DESC",
}
...
func (p *Pages) CatListHandler(c sevcord.Ctx, params string) {
	...
	err = p.db.Select(&cats, `SELECT name, cardinality(elements) length, COALESCE(array_length(elements & (SELECT inv FROM inventories WHERE guild=$1 AND "user"=$4), 1), 0) common FROM categories WHERE guild=$1 ORDER BY `+catListSortSql[parts[2]]+` LIMIT $2 OFFSET $3`, c.Guild(), length, length*page, parts[1])
	if err != nil {
		p.base.Error(c, err)
		return
	}
