...
func (e *Elements) IdeaHandler(c sevcord.Ctx, params string) {
	...
	for i := 0; i < MaxIdeaReqs; i++ {
		...
		var err error
		if parts[1] == "" {
			err = e.db.Select(&els, `SELECT id FROM elements WHERE guild=$1 ORDER BY RANDOM() LIMIT $2`, c.Guild(), cnt)
		} else {
			query, ok := e.base.CalcQuery(c, parts[1]) // TODO: Move this out of the loop
			if !ok {
				return
			}
			err = e.db.Select(&els, `SELECT id FROM elements WHERE guild=$1 AND id=ANY($2) ORDER BY RANDOM() LIMIT $3`, c.Guild(), pq.Array(query.Elements), cnt)
		}
		if err != nil {
			e.base.Error(c, err)
		}
