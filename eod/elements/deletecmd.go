...
func (e *Elements) deleteComboCmd(c sevcord.Ctx, opts []any) {
	...
	err = e.db.QueryRow(`SELECT treesize FROM elements WHERE id=$1 AND guild=$2`, opts[0].(int64), c.Guild()).Scan(&currtreesize)
	if err != nil {
		e.base.Error(c, err)
		return
	}
	...
	) SELECT COUNT(*), EXISTS(SELECT 1 FROM parents WHERE id=$3) FROM parents WHERE id>0`, c.Guild(), pq.Array(els), opts[0].(int64)).Scan(&treesize, &loop)
	// Force update since only one combo left
	if !loop {
		// Update parents
